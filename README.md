...
oss cp s3://chengdudata/cd_brain/data/SIGNS\ dataset.zip ./dataset/
```
...
Once the download is complete, move the dataset into `dataset/SIGNS`.
```shell
cd dataset
unzip SIGNS\ dataset.zip
mv SIGNS\ dataset SIGNS
```
Run the script `build_dataset.py` which will resize the images to size `(64, 64)` and then convert to `.nori` format. 

The new resized dataset will be located by default in `s3://chengdudata/cd_brain/{usr_name}/dataset/64x64_SIGNS` 
and `nori ids` will be stored in `dataset/64x64_SIGNS/{train/val}/{train/val}.pickle`:

```bash
cd dataset
python build_dataset.py --data_dir SIGNS --output_dir 64x64_SIGNS
```
...
```
python common/illustrate_results.py
```
...
```
python3 evaluate.py --restore_file experiments/experiment_learning_rate/exp_1/model_latest.pth
```

## Init Your Own Project
we provide a simple script to init the git repo, project name, just run:
```python
chmod +x init.sh
# replace project name with yours, e.g., souce init.sh gyroFlow
source init.sh [project name]
```
...
>>>>>>> 1293948c2b1e17a12ee41d28c07169749c819722
